...
export const PaymentScreen = (props: any) => {
  ...
  const _handleAppStateChange = (nextAppState: any) => {
    if (
      ...
      verifyPaymentLink(body, jwt)
        .then((res:VerifyTransactionResponse) => {
          if (res.success) {
  ...
  return (
    <Layout style={styles.container}>
      <Text category="h6" status="warning" style={{ marginBottom: 25 }}>
        Your payment has been initiated, please click on the button below to
      ...
      <Text status="danger" style={{ marginTop: 50 }}>
        Please do not press the back button until payment is complete{" "}
      </Text>
    </Layout>
  );
};
